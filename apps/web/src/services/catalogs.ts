...
export async function getDamageSpaces() {
  return fetchAllSorted<DamageSpace>("damage_spaces", {
    select: "id, name, description, is_active, is_wildcard, applicable_classifications, created_at, updated_at",
    eq: { is_active: true },
  });
}
...
// ═══════════════════════════════════════════════════════════════
// Resolución jerárquica de ubicación (comuna → ciudad → región → país)
// ═══════════════════════════════════════════════════════════════
...
/**
 * Resuelve la jerarquía completa de ubicación desde el nombre de una comuna.
 * comuna → city_id → ciudad → region_id → región → country_id → país
 * Retorna todos los IDs y nombres, o nulls si no encuentra la comuna.
 */
export async function resolveCommuneHierarchy(communeName: string): Promise<ResolvedLocation> {
  const empty: ResolvedLocation = {
    communeId: null, cityId: null, regionId: null, countryId: null,
    communeName: null, cityName: null, regionName: null, countryName: null,
  };
  if (!communeName || !communeName.trim()) return empty;

  // 1. Buscar comuna por nombre (case-insensitive, sin acentos)
  const communes = await fetchAll<Commune & { city: { id: string; name: string; region_id: string } | null }>(
    "communes",
    {
      select: "id, city_id, name, is_active, city:cities!communes_city_id_fkey(id, name, region_id)",
      eq: { is_active: true },
    },
  );
  const normalizedSearch = communeName.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
  ...
  if (!commune) return empty;

  // 2. Buscar ciudad
  let city: { id: string; name: string; region_id: string } | null = commune.city;
  if (!city && commune.city_id) {
    const cities = await fetchAll<City>("cities", {
      select: "id, region_id, name, is_active",
      eq: { id: commune.city_id, is_active: true },
      limit: 1,
    });
    city = cities[0] ? { id: cities[0].id, name: cities[0].name, region_id: cities[0].region_id } : null;
  }
  if (!city) {
    return {
      ...empty,
  ...
  // 3. Buscar región
  const regions = await fetchAll<Region>("regions", {
    select: "id, country_id, code, name, is_active",
    eq: { id: city.region_id, is_active: true },
    limit: 1,
  });
  ...
  // 4. Buscar país
  const countries = await fetchAll<Country>("countries", {
    select: "id, code, name",
