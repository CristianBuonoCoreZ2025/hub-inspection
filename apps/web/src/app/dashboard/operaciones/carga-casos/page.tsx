...
import {
  CASOS_FIELDS,
  autoDetectCasosMapping,
  applyCasosMapping,
  validateCasosRow,
  type ColumnMapping,
  type LearnedCasosFieldMapping,
} from "@/lib/claim-import/schema-casos";
import {
...
export default function CargaCasosPage() {
  ...
  const refFields = useMemo(() => [
    { fieldKey: "insuranceCompany", label: "Compañía Seguros", options: insuranceCompanies ?? [] },
    ...
    { fieldKey: "insuranceProduct", label: "Ramo/Producto", options: insuranceProducts ?? [] },
    ...
    { fieldKey: "currency", label: "Moneda", options: (currencies ?? []).map(c => ({ id: c.id, name: c.code })) },
  ...
  const handleFile = useCallback((f: File) => {
    ...
    reader.onload = (e) => {
      ...
      })) as LearnedCasosFieldMapping[];
      const autoMapping = autoDetectCasosMapping(headers, learned);
  ...
  const parsedRows = useMemo(() => {
    return rawRows.map((raw, idx) => {
      const data = applyCasosMapping(raw, mapping);
      ...
      const { valid, errors } = validateCasosRow(data, mapping);
  ...
  const stagingMutation = useMutation({
    mutationFn: async () => {
      ...
      const dups = await checkCasosDuplicates(tenantCompanyId, rowsToCheck);
      setDuplicates(dups);
      ...
      if (rowsToStage.length === 0) throw new Error("No hay casos nuevos para cargar (todos son duplicados)");
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          const claim = await createClaimFromCaso({
            ...
            rut: String(d.rut || ""),
            insuredAddress: String(d.insuredAddress || ""),
            claimAddress: String(d.claimAddress || ""),
            commune: String(d.commune || ""),
            city: String(d.city || ""),
            insuredPhone: String(d.insuredPhone || ""),
          ...
          await markStagingError(staging.id, (err as Error).message);
          error++;
        }
        setProgress({ current: i + 1, total: rows.length, success, error });
      }
      ...
      <div className="app-grid-header">
        <div className="app-grid-header-left">
          ...
          <div className="app-grid-title-row">
            <h1 className="app-page-title shrink-0">Carga Casos</h1>
          </div>
        </div>
      </div>
      ...
      <div className="app-panel">
        ...
        {step === "upload" && (
          ...
          >
            ...
            <p className="app-body text-muted-foreground mb-2">Arrastra el Excel de Casos aquí</p>
                  ...
                  {Object.entries(effectiveFixedValues).length > 0 && (
                    <div className="bulk-fixed-values-existing">
                      {Object.entries(effectiveFixedValues).map(([fk, fv]) => {
                        ...
                        const field = CASOS_FIELDS.find((f) => f.key === fk);
              ...
              {canCreate("operaciones") && stagingRows.length > 0 && (
                <Tooltip>
                  ...
                  {(!effectiveFixedValues.insuranceProduct?.catalogUuid ||
                    ...
                    !effectiveFixedValues.insuranceCompany?.catalogUuid) && (
                    <TooltipContent side="top">
                      <p>Falta configurar como valor fijo:</p>
                      {!effectiveFixedValues.insuranceCompany?.catalogUuid && <p>• Compañía Seguros</p>}
                      {!effectiveFixedValues.insuranceProduct?.catalogUuid && <p>• Ramo/Producto</p>}
                      {!effectiveFixedValues.adjuster?.catalogUuid && <p>• Ajustador/Liquidador</p>}
                    </TooltipContent>
                  )}
                </Tooltip>
              )}
