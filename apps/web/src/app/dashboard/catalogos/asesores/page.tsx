...
import { Search, Pencil, Ban, UserCheck } from "lucide-react";
...
export default function AsesoresPage() {
 ...
 : paginatedData.map((a) => (
 <tr key={a.id}>
 <td><StatusBadge status={a.is_active ? "active" : "inactive"} label={a.is_active ? "Activo" : "Inactivo"} /></td>
 ...
 {canDelete("catalogos") && (
 <button type="button" className="btn-icon-sm btn-danger-hover" onClick={async () => { const ok = await confirm({ title: "Desactivar", description: "¿Desactivar este asesor?", confirmLabel: "Desactivar", destructive: true }); if (!ok) return; deleteMutation.mutate(a.id); }}><Ban className="h-4 w-4" /></button>
 )}
 </div>
 </td>
 </tr>
 ...
 <div className="flex h-8 w-8 items-center justify-center rounded-lg icn-sky text-white shadow-sm"><UserCheck className="h-4 w-4" /></div>
