...
/**
 * Sube un documento del siniestro a R2 SIN optimizar (raw, rápido).
 * La optimización se hace después en background.
 */
export async function uploadClaimDocumentRaw(
  claimId: string,
  buffer: Buffer,
  contentType: string,
  ext: string
  ...
  // 3. Construir path (sin optimizar — usa extensión original)
  ...
  logger.info("Documento de siniestro subido (raw)", {
    component: "claim-upload",
    action: "claim.doc.upload.raw",
...
/**
 * Re-sube un documento optimizado a R2, reemplazando la versión raw.
 * Se usa después de la subida inicial para optimizar en background.
 */
export async function reuploadClaimDocumentOptimized(
  claimId: string,
  seq: number,
  buffer: Buffer,
  contentType: string,
  ...
  const key = claimDocumentPath(claim.liquidation_number, String(seq), optimized.ext);

  // Subir a R2 (reemplaza el archivo anterior si la key es la misma)
  const url = await uploadToR2(optimized.buffer, key, optimized.mimeType);

  logger.info("Documento de siniestro optimizado y re-subido", {
    component: "claim-upload",
    action: "claim.doc.optimize",
    metadata: {
      claimId,
      seq,
      key,
