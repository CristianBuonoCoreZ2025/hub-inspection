...
const ACTION_FEATURE_SELECT =
  `id, name, has_specific_screen, has_control, has_issue, has_review, has_approve, is_active, sort_order, screen_id, created_at, updated_at, screen:gestion_screens!action_features_screen_id_fkey(id, code, name, description, icon, form_schema), characteristics:characteristic!characteristic_action_feature_id_fkey(${CHARACTERISTIC_SELECT})`;
