...
import { Plus, Search, Pencil, Trash2, Home, Boxes } from "lucide-react";
...
export default function PropertyClassificationPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Clasificacion Bien creado");
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Clasificacion Bien actualizado");
      queryClient.invalidateQueries({ queryKey: ["clasificacion_bien"] });
      setOpen(false);
      setEditingId(null);
      setFormData({"name":"","description":""});
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Clasificacion Bien desactivado");
      queryClient.invalidateQueries({ queryKey: ["clasificacion_bien"] });
    },
    onError: (err: Error) => toast.error(err.message),
  });
      ...
      <div className="app-page-header">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            {canCreate("catalogos") && (
              <Button onClick={() => { setEditingId(null); setFormData({"name":"","description":""}); setOpen(true); }} className="liquid-button">
        ...
        <DialogContent className="modal-md" showCloseButton={false}>
          <div className="modal-header">
            <DialogTitle className="modal-title flex items-center gap-2.5">
              ...
              {editingId ? "Editar" : "Nuevo"} Clasificacion Bien
            </DialogTitle>
          </div>
