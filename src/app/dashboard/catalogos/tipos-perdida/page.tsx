...
import { getLossTypes, createLossType, updateLossType, deleteLossType } from "@/services/catalogs";
...
export default function LossTypePage() {
  ...
  const { data: items, isLoading } = useQuery({
    queryKey: ["tipos_perdida"],
    queryFn: getLossTypes,
  });

  const createMutation = useMutation({
    mutationFn: createLossType,
    onSuccess: () => {
      toast.success("Tipos Perdida creado");
      queryClient.invalidateQueries({ queryKey: ["tipos_perdida"] });
      setOpen(false);
      setFormData({"name":""});
    },
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, input }: { id: string; input: Parameters<typeof updateLossType>[1] }) => updateLossType(id, input),
    onSuccess: () => {
      toast.success("Tipos Perdida actualizado");
      queryClient.invalidateQueries({ queryKey: ["tipos_perdida"] });
      setOpen(false);
      setEditingId(null);
      setFormData({"name":""});
  ...
  const deleteMutation = useMutation({
    mutationFn: deleteLossType,
    onSuccess: () => {
      toast.success("Tipos Perdida desactivado");
      queryClient.invalidateQueries({ queryKey: ["tipos_perdida"] });
    },
    onError: (err: Error) => toast.error(err.message),
  });
  ...
  return (
    <div className="app-page">
      <header className="app-page-header">
        <h1 className="app-page-title">Tipos Perdida</h1>
        ...
        <DialogContent className="modal-md" showCloseButton={false}>
          <div className="modal-header">
            <DialogTitle className="modal-title flex items-center gap-2.5">
              ...
              {editingId ? "Editar" : "Nuevo"} Tipos Perdida
            </DialogTitle>
          </div>
