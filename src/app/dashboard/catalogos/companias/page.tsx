...
import { getInsuranceCompanies, createInsuranceCompany, updateInsuranceCompany, deleteInsuranceCompany } from "@/services/catalogs";
...
export default function CompaniasPage() {
  ...
  const [formData, setFormData] = useState({ name: "", rut: "", address: "", line_of_business: "", code: "", type: "Generales" });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { toast.success("Compañia creada"); queryClient.invalidateQueries({ queryKey: ["insurance-companies"] }); setOpen(false); setFormData({ name: "", rut: "", address: "", line_of_business: "", code: "", type: "Generales" }); },
  ...
  return (
    <div className="app-page">
      ...
      <div className="app-toolbar">
        ...
        <Button onClick={() => { setEditingId(null); setFormData({ name: "", rut: "", address: "", line_of_business: "", code: "", type: "Generales" }); setOpen(true); }} className="btn-create btn-sm">
      ...
      <div className="app-data-table-wrap">
        <table className="app-data-table">
          <thead><tr><th className="w-10"></th><th>Nombre</th><th>RUT</th><th>Direccion</th><th>Ramo</th><th>Codigo</th><th>Tipo</th><th className="w-[80px]"></th></tr></thead>
          <tbody>
            {isLoading ? <tr><td colSpan={8} className="text-center text-muted-foreground py-4">Cargando...</td></tr>
            : filtered?.length === 0 ? <tr><td colSpan={8} className="text-center text-muted-foreground py-4">No se encontraron registros.</td></tr>
            : filtered?.map((c) => (
              <tr key={c.id}>
                ...
                <td>
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" className="btn-neutral btn-icon" onClick={() => { setEditingId(c.id); setFormData({ name: c.name, rut: c.rut || "", address: c.address || "", line_of_business: c.line_of_business || "", code: c.code || "", type: c.type || "Generales" }); setOpen(true); }}><Pencil className="h-4 w-4" /></Button>
