...
import { getBrokers, createBroker, updateBroker, deleteBroker } from "@/services/catalogs";
...
export default function CorredoresPage() {
  ...
  const [formData, setFormData] = useState({ name: "", rut: "", address: "", contact: "" });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { toast.success("Corredor creado"); queryClient.invalidateQueries({ queryKey: ["brokers"] }); setOpen(false); setFormData({ name: "", rut: "", address: "", contact: "" }); },
  ...
  return (
    <div className="app-page">
      ...
      <div className="app-toolbar">
        ...
        <Button onClick={() => { setEditingId(null); setFormData({ name: "", rut: "", address: "", contact: "" }); setOpen(true); }} className="btn-create btn-sm">
      ...
      <div className="app-data-table-wrap">
        <table className="app-data-table">
          <thead><tr><th className="w-10"></th><th>Nombre</th><th>RUT</th><th>Direccion</th><th>Contacto</th><th className="w-[80px]"></th></tr></thead>
          <tbody>
              ...
              <tr key={b.id}>
                ...
                <td>
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" className="btn-neutral btn-icon" onClick={() => { setEditingId(b.id); setFormData({ name: b.name, rut: b.rut || "", address: b.address || "", contact: b.contact || "" }); setOpen(true); }}><Pencil className="h-4 w-4" /></Button>
        ...
        <DialogContent className="modal-md" showCloseButton={false}>
          <div className="modal-header">
            <DialogTitle className="modal-title flex items-center gap-2.5">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-[#0095DA] to-[#005BBB] text-white shadow-sm"><Briefcase className="h-4 w-4" /></div>
