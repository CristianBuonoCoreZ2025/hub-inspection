...
import { getAreas, createArea, updateArea, deleteArea } from "@/services/catalogs";
...
export default function AreaPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, input }: { id: string; input: Parameters<typeof updateArea>[1] }) => updateArea(id, input),
    onSuccess: () => {
      toast.success("Areas actualizado");
      queryClient.invalidateQueries({ queryKey: ["areas"] });
      setOpen(false);
      setEditingId(null);
      setFormData({"name":""});
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Areas desactivado");
      queryClient.invalidateQueries({ queryKey: ["areas"] });
    },
    onError: (err: Error) => toast.error(err.message),
  });
  ...
  return (
    <div className="app-page">
      <header className="app-page-header">
        <h1 className="app-page-title">Areas</h1>
