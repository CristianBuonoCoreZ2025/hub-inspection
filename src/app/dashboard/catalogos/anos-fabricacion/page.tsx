...
import { getManufactureYears, createManufactureYear, updateManufactureYear, deleteManufactureYear } from "@/services/catalogs";
...
export default function ManufactureYearPage() {
  ...
  const { data: items, isLoading } = useQuery({
    queryKey: ["anos_fabricacion"],
    queryFn: getManufactureYears,
  });

  const createMutation = useMutation({
    mutationFn: createManufactureYear,
    onSuccess: () => {
      toast.success("Ano Fabricacion creado");
      queryClient.invalidateQueries({ queryKey: ["anos_fabricacion"] });
      setOpen(false);
      setFormData({"name":""});
    },
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, input }: { id: string; input: Parameters<typeof updateManufactureYear>[1] }) => updateManufactureYear(id, input),
    onSuccess: () => {
      toast.success("Ano Fabricacion actualizado");
      queryClient.invalidateQueries({ queryKey: ["anos_fabricacion"] });
      setOpen(false);
      setEditingId(null);
      setFormData({"name":""});
  ...
  const deleteMutation = useMutation({
    mutationFn: deleteManufactureYear,
    onSuccess: () => {
      toast.success("Ano Fabricacion desactivado");
      queryClient.invalidateQueries({ queryKey: ["anos_fabricacion"] });
    },
    onError: (err: Error) => toast.error(err.message),
  });
  ...
  return (
    <div className="app-page">
      <header className="app-page-header">
        <h1 className="app-page-title">Ano Fabricacion</h1>
      ...
      <div className="app-toolbar">
        ...
        <Button
          onClick={() => { setEditingId(null); setFormData({"name":""}); setOpen(true); }}
        ...
        <DialogContent className="modal-md" showCloseButton={false}>
          <div className="modal-header">
            <DialogTitle className="modal-title flex items-center gap-2.5">
              ...
              {editingId ? "Editar" : "Nuevo"} Ano Fabricacion
            </DialogTitle>
          </div>
