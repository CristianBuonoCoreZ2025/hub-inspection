...
import { getHousingTypes, createHousingType, updateHousingType, deleteHousingType } from "@/services/catalogs";
...
export default function HousingTypePage() {
  ...
  const { data: items, isLoading } = useQuery({
    queryKey: ["tipos_vivienda"],
    queryFn: getHousingTypes,
  });

  const createMutation = useMutation({
    mutationFn: createHousingType,
    onSuccess: () => {
      toast.success("Tipos Vivienda creado");
      queryClient.invalidateQueries({ queryKey: ["tipos_vivienda"] });
      setOpen(false);
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, input }: { id: string; input: Parameters<typeof updateHousingType>[1] }) => updateHousingType(id, input),
    onSuccess: () => {
      toast.success("Tipos Vivienda actualizado");
      queryClient.invalidateQueries({ queryKey: ["tipos_vivienda"] });
      setOpen(false);
      setEditingId(null);
      setFormData({"name":"","description":""});
  ...
  const deleteMutation = useMutation({
    mutationFn: deleteHousingType,
    onSuccess: () => {
      toast.success("Tipos Vivienda desactivado");
      queryClient.invalidateQueries({ queryKey: ["tipos_vivienda"] });
    },
    onError: (err: Error) => toast.error(err.message),
  });
  ...
  return (
    <div className="app-page">
      <header className="app-page-header">
        <h1 className="app-page-title">Tipos Vivienda</h1>
        ...
        <DialogContent className="modal-md" showCloseButton={false}>
          <div className="modal-header">
            <DialogTitle className="modal-title flex items-center gap-2.5">
              ...
              {editingId ? "Editar" : "Nuevo"} Tipos Vivienda
            </DialogTitle>
          </div>
