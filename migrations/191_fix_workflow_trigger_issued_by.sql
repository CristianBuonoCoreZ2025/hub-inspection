-- ═══════════════════════════════════════════════════════════════
-- Migración 191: Fix trigger execute_workflow_on_status_change
-- ═══════════════════════════════════════════════════════════════
--
-- PROBLEMA:
-- La función execute_workflow_on_status_change() usaba NEW.issued_by
-- y NEW.created_by / NEW.updated_by en el COALESCE para el campo
-- created_by del INSERT en claim_actions. Pero la tabla claims NO tiene
-- esos campos (issued_by, created_by, updated_by), lo que causaba el
-- error "record 'new' has no field 'issued_by'" al cambiar el estado
-- del siniestro, impidiendo que el workflow creara las gestiones
-- automáticas.
--
-- SOLUCIÓN:
-- Usar NULL directamente como created_by de las gestiones automáticas
-- creadas por el workflow. El created_by de estas gestiones no es
-- crítico porque el origen 'W' (workflow) ya indica que fueron creadas
-- automáticamente por el sistema.
--
-- IMPACTO:
-- - Las gestiones automáticas ahora se crean correctamente al cambiar
--   el estado del siniestro.
